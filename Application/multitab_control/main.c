...
// int soundthreadid;
// char music_name[100]="be.wav";
// void *sound_thread(int *);
            ...
            if(strchr(bufmsg, 'C') != NULL){
               ...
               for(; index < q.size; index++){
                  if(index == music_index){
                  	...
                  	{
                     printf("PLAY : %s\n", Peek(&q));
                     file_name = (char *)malloc(strlen(Peek(&q) + 9));
                     sprintf(file_name, "Playlist/%s", Peek(&q));
                     // system(file_name);
                     execl("/usr/bin/aplay", "aplay", file_name, NULL);
                     // strcpy(music_name,file_name);
                      free(file_name);
                  	}
                  	wait(&status);
                  	printf("music end\n");	
...
// void *sound_thread(int *arg)
// {
// 	execl("/usr/bin/aplay", "aplay", music_name, NULL);
// 	return (void *)0;
// }
